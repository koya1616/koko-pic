...
const RequestCreationScreen: React.FC<{ navigateTo: (screen: any) => void }> = ({ navigateTo }) => {
  ...
  const [requestText, setRequestText] = useState('駅前の混雑状況がわかる写真1枚ください');
  ...
  return (
    <div className="flex flex-col h-full bg-gray-50 p-4">
      ...
      <div className="bg-gray-200 rounded-lg h-40 mb-4 flex items-center justify-center">
        ...
        <div className="absolute bg-indigo-600 w-6 h-6 rounded-full transform -translate-x-1/2 -translate-y-1/2" style={{ left: '50%', top: '50%' }}></div>
      </div>
      ...
      {/* Request Content */}
      <div className="mb-4">
        <label className="block text-sm font-medium text-gray-700 mb-1">依頼内容（必須）</label>
        <textarea
          value={requestText}
      ...
      {/* Reward Selection */}
      <div className="mb-4">
        <label className="block text-sm font-medium text-gray-700 mb-1">報酬</label>
        ...
        <div className="mt-2 flex items-center">
          <input
            type="radio"
            name="reward"
            checked={reward === parseInt(otherReward)}
            onChange={() => setReward(parseInt(otherReward) || 0)}
            className="form-radio h-4 w-4 text-indigo-600"
          />
      ...
      {/* Options */}
      <div className="mb-6">
        <label className="block text-sm font-medium text-gray-700 mb-1">オプション</label>
        <div className="space-y-2">
          <label className="inline-flex items-center">
            <input
              type="checkbox"
              checked={options.noPeople}
              onChange={(e) => setOptions({...options, noPeople: e.target.checked})}
          ...
          <label className="inline-flex items-center">
            <input
              ...
              onChange={(e) => setOptions({...options, todayOnly: e.target.checked})}
              className="form-checkbox h-4 w-4 text-indigo-600"
            />
      ...
      <div className="mt-auto space-y-3">
        ...
        >
          💳 今すぐ依頼（¥{reward || parseInt(otherReward) || 300}）
        </button>
      </div>
    </div>
  );
